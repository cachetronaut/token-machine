from token_machine.metrics.aggregate import dashboard_summary, rollup_events
...
def test_tool_mix_uses_observed_action_labels_without_merging_kinds() -> None:
    ...
    assert observed_tool_counts(events) == {"Custom Repo Scan": 1}
    assert observed_executable_counts(events) == {"Rg": 1}
    assert observed_skill_counts(events) == {"frontend-design": 1}
    assert [item.category for item in tool_mix(events)] == [
        "Custom Repo Scan",
        "Exec: Rg",
...
def test_executable_counts_include_commands_attached_to_tool_calls() -> None:
    ...
    assert data.command_calls == 1
    assert data.executables == {"Git": 1}
    assert data.clis == {"Git": 1}


def test_command_counts_deduplicate_matching_tool_and_command_events() -> None:
    events = [
        AnalyticsEvent(
            event_id="e1",
            event_type=EventType.TOOL_CALL,
    ...
    data = dashboard_summary(events)

    assert data.command_calls == 1
    assert data.executables == {"Uv": 1}


def test_legacy_skill_tool_counts_as_skill_until_reingested() -> None:
    events = [
        AnalyticsEvent(
            event_id="e1",
            event_type=EventType.TOOL_CALL,
    ...
    assert rollup.tools == {}
    assert rollup.skill_calls == 1
    assert rollup.skills == {"Skill": 1}
    assert data.tools == {}
    assert data.skill_calls == 1
    assert data.skills == {"Skill": 1}
...
def test_tool_mix_populates_descriptions() -> None:
    ...
    bash_item = next(item for item in mix if item.category == "Exec: Bash")
...
def test_tool_mix_prefers_shortest_command() -> None:
    ...
    mix = tool_mix(events)
    rg_item = next(item for item in mix if item.category == "Exec: Rg")
...
def test_tool_mix_truncates_long_descriptions() -> None:
    ...
    mix = tool_mix(events)
    item = next(item for item in mix if item.category == "Exec: Long Cli")
