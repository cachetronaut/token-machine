...
import type { CountMap, ModelProfile } from "./types.js";
...
export function renderModelProfiles(rows: ModelProfile[]) {
  ...
  root.innerHTML = visibleRows
    ...
    .map((row) => {
      ...
      const effort =
        row.reasoning_level && row.reasoning_level !== "not in log" ? row.reasoning_level : "";
...
function renderProviderLogo(row: ModelProfile) {
...
function renderModelHero(row: ModelProfile) {
...
function modelCardColor(row: ModelProfile) {
...
function renderRankMedallions(row: ModelProfile, limit = 3) {
...
function renderTopList(title: string, values: CountMap, total: number, limit = 3) {
