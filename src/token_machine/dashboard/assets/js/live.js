...
export function renderLiveError() {
    ...
    if (led)
        led.className = "live-led live-led-error";
    const lanesRoot = optionalElement("live-lanes");
    if (!lanesRoot)
        return;
...
function setStatusLine(data, activeSnapshots, snapshots) {
    ...
    if (activeSnapshots.length)
        parts.push(`${fmt.format(activeSnapshots.length)} active`);
    if (activeApps)
        parts.push(activeApps);
    if (staleCount)
        parts.push(`${fmt.format(staleCount)} stale`);
    if (!parts.length && snapshots.length)
        parts.push("No active sessions");
    if (!parts.length)
        parts.push("Awaiting sessions");
    if (generatedAt)
        parts.push(`updated ${generatedAt}`);
...
function contextLevel(percent, hasWindow) {
    if (!hasWindow)
        return "live-context-missing";
    if (percent >= 95)
        return "live-context-critical";
    if (percent >= 75)
        return "live-context-warn";
...
function limitDisplayName(name) {
    const key = String(name || "")
        .toLowerCase()
        .replace(/[\s-]+/g, "_");
    if (["primary", "five_hour", "5h", "current"].includes(key))
        return "current";
    if (["secondary", "seven_day", "7d", "weekly"].includes(key))
        return "weekly";
    return name || "limit";
}
...
function liveActionKind(tool) {
    ...
    if (["exec_command", "run_shell_command", "bash"].includes(name))
        return "command";
    return "tool";
}
function snapshotSubagentCount(snapshot) {
    return Math.max(subagentCount(liveActions(snapshot)), Number(snapshot.current_metrics?.subagent_sessions || 0));
}
...
function sessionTokens(snapshot) {
    return Number(snapshot.current_metrics?.session_total_tokens ||
