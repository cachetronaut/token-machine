...
function chartSvg(id, points, getValue, lineColor, _fillColor, labelKey, options, meta) {
    const width = 760;
    const height = 260;
    const pad = { top: 28, right: 30, bottom: 44, left: 62 };
    const innerW = width - pad.left - pad.right;
...
function chartOverlay(meta, points) {
    if (!points.length)
        return "";
    const { xy, viewW, viewH, values } = meta;
    if (!xy || !viewW || !viewH || !values)
        return "";
    const beads = xy
        .map(([x, y], index) => {
...
function animateSignalChart(root) {
    ...
    const head = root.querySelector(".chart-marker");
    const beadsLayer = root.querySelector(".chart-beads");
    const valueTag = root.querySelector(".chart-value");
    ...
    if (!line || !clip || !head || points.length === 0)
        return;
