...
async function refresh() {
  try {
    ...
    renderChart("daily-chart", data.daily, (row) => row.summary?.tokens?.total_tokens || 0, "#43c7b7", "#43c7b7", "day");
    renderChart("hourly-chart", data.hourly, (row) => row.summary?.event_count || 0, "#f6c453", "#f6c453", "hour");
    renderModelDistribution(summary.models);
    renderAppLegend(summary.sources);
    renderBars("tools", summary.tools, true);
    renderBars("clis", summary.clis, true);
    renderModelProfiles(data.model_profiles);
    renderSessions(data.recent_sessions);
    const updatedAt = new Date(data.generated_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
